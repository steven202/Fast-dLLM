#!/bin/bash
# LLaDA-1.5 Baseline (Fast-dLLM), n=200
# GPU-agnostic: CUDA_VISIBLE_DEVICES set by launcher
...
for ds in "gsm8k 5 200" "humaneval 0 164" "minerva_math500 4 200" "mbpp 3 200"; do
  ...
  echo "--- $task (${fs}-shot) ---"
  accelerate launch $FD/eval_llada.py --tasks $task --num_fewshot $fs --model llada_dist --model_args ${BASE} --batch_size 1 --output_path $out --log_samples --limit $lim --confirm_run_unsafe_code 2>&1 | tee -a $out.log
done
echo "=== LLaDA-1.5 Baseline DONE ==="
