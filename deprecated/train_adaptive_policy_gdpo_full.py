...
class RolloutResult:
    ...
    # We store the pre-calculated decoupled advantages here
...
def get_transfer_index(
    ...
    # Store original shape: [Batch, SeqLen] (e.g., [1, 294])
    original_shape = probs.shape[:-1] 
    vocab_size = probs.shape[-1]
    
    # Flatten Batch and Seq dimensions: [Batch * SeqLen, Vocab]
    ...
    # Recalculate probabilities for the sampled tokens
    # gather expects index to have same dimensions as input, so we unsqueeze x0
    ...
    if threshold is not None:
        ...
        # Ensure we at least accept the best token if everything fails threshold
...
def rollout_llada(
    ...
    return RolloutResult(
        ...
        a_weighted_sum=torch.tensor(0.0) # Placeholder
    )
...
def main():
    ...
    # [MODIFIED] Added batch size to enable GDPO Batch-wise Normalization
    ...
    if use_wandb:
        try:
            ...
            wandb = _wandb
            wandb.init(project="adaptive-dllm", dir="./wandb_log", config=vars(args), name=datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
        except Exception:
            use_wandb = False
            print("wandb import failed.")

    # [MODIFIED] Batch Buffer to store rollouts for Batch-wise Normalization
    batch_buffer = []
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        for i, prompt in enumerate(tqdm(prompts, desc="Prompts")):
            ...
            # --- Collection Phase (Single Prompt Group) ---
            # Generate rollouts for this ONE prompt
            r_qual_list = []
            r_speed_list = []
            r_ccd_list = []
            ...
            for _ in range(args.rollouts):
                if args.model_type == "llada":
                    rollout = rollout_llada(
                        model, policy_net, tokenizer, prompt, ar_guidance_model,
            ...
            # Normalize each reward independently within this group (Prompt)
            ...
            # We calculate the weighted sum BUT we do not update yet.
            # We must wait to normalize this sum across the Batch.
            a_weighted_sum = args.w1 * a_q + args.w2 * a_s - args.lambda_ccd * a_ccd
            
            # Store calculated advantages back into rollouts for batch processing
            for idx, r in enumerate(rollouts):
                r.a_weighted_sum = a_weighted_sum[idx]
            
            # Add this group's rollouts to the batch buffer
            batch_buffer.extend(rollouts)

            # --- GDPO Step 3: Batch-wise Optimization  ---
            # Only update when we have accumulated enough prompts (Batch Size)
            if len(batch_buffer) >= (args.batch_size * args.rollouts):
                
                # 1. Collect all "summed advantages" from the batch
                all_a_sums = torch.stack([r.a_weighted_sum for r in batch_buffer])
                
                # 2. Batch-wise Normalization [cite: 57, 230]
                # "applying batch-wise advantage normalization to preserve a stable numerical range"
                batch_mean = all_a_sums.mean()
                batch_std = all_a_sums.std()
                
                # Normalize the advantages across the *entire batch*
                if batch_std > 1e-6:
                    all_a_final = (all_a_sums - batch_mean) / (batch_std + 1e-8)
                else:
                    all_a_final = all_a_sums - batch_mean # Fallback

                # 3. Compute Loss & Update
                losses = []
                for idx, rollout in enumerate(batch_buffer):
                    # Re-compute log probs for PPO
                    ...
                    # Use the Batch-Normalized Advantage
                    loss = ppo_loss(new_logprob, rollout.logprob.detach(), all_a_final[idx].detach(), args.ppo_clip)
                    losses.append(loss)
                ...
                if i == 0 or (i + 1) % 1 == 0:
                    print(f"Step {i} | Loss: {loss_total.item():.4f} | R_qual: {r_qual_tensor.mean():.4f} | R_speed: {r_speed_tensor.mean():.4f} | R_CCD: {r_ccd_tensor.mean():.4f} | a_q: {a_q.mean():.4f} | a_s: {a_s.mean():.4f} | a_ccd: {a_ccd.mean():.4f} | Time: {time.time() - start_time:.2f}s")
                if use_wandb:
                    wandb.log({
