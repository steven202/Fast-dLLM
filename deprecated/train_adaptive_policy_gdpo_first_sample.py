...
# [MODIFIED] Added temperature to force exploration in block length
def sample_block_len(
    ...
    # Apply temperature to increase entropy (exploration)
...
def rollout_dream(
    ...
    block_len_max: int,
    threshold: float,
    temperature: float = 1.0 # [MODIFIED] Added temp arg
) -> RolloutResult:
    device = model.device
    input_ids = tokenizer(prompt, return_tensors="pt").input_ids.to(device)
...
def main():
    ...
    # [MODIFIED] Added explicit generation temperature for rollouts
    ...
    if use_wandb:
        try:
            ...
            wandb = _wandb
            wandb.init(project="adaptive-dllm", dir="./wandb_log", config=vars(args), name=f"gdpo_{args.model_type}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        for i, prompt in enumerate(tqdm(prompts, desc="Prompts")):
            ...
            for _ in range(args.rollouts):
                if args.model_type == "llada":
                    rollout = rollout_llada(
                        ...
                        args.block_len_max,
                        args.threshold,
                        args.temperature # [MODIFIED] Pass temp
                    )
                else:
            ...
            # [MODIFIED] Safe Normalization to prevent Zero/NaN when batch is identical
