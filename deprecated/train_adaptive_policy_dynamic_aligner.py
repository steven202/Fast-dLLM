...
class LogitAligner:
    """Helper to map logits from one tokenizer vocab to another via string decoding/encoding."""
    def __init__(self, src_tokenizer, tgt_tokenizer):
        self.src = src_tokenizer
        self.tgt = tgt_tokenizer
        self.cache = {} # source_id -> target_id

    def align(self, src_logits: torch.Tensor, tgt_vocab_size: int) -> torch.Tensor:
        """
        src_logits: [1, vocab_src]
        Returns: [1, vocab_tgt] with sparse populated values mapped from top-k src logits.
        """
        # We only map the top predictions to save time
        k = 64
        topk_vals, topk_indices = torch.topk(src_logits, k=min(k, src_logits.shape[-1]), dim=-1)
        
        tgt_logits = torch.full((src_logits.size(0), tgt_vocab_size), float('-inf'), device=src_logits.device, dtype=src_logits.dtype)
        
        src_ids = topk_indices[0].cpu().tolist()
        vals = topk_vals[0]
        ...
        for i, sid in enumerate(src_ids):
            if sid in self.cache:
                ...
                # Decode -> Encode
                token_str = self.src.decode([sid])
                # We want the primary definition of this string in target vocab
                # encode returning multiple tokens is problematic, typically we accept the first if it matches
                t_ids = self.tgt.encode(token_str, add_special_tokens=False)
                if len(t_ids) > 0:
                    tid = t_ids[0]
                else:
                    tid = None
            ...
            if tid is not None and tid < tgt_vocab_size:
                # If multiple source tokens map to same target, take max prob
                tgt_logits[0, tid] = max(tgt_logits[0, tid], vals[i])
    ...
    while current_len < gen_length:
        ...
        if ar_guidance_model is not None:
            if hasattr(ar_guidance_model, "logit_aligner") and ar_guidance_model.logit_aligner is not None:
                aligner = ar_guidance_model.logit_aligner
                # [FIX] Transcode context (Backbone -> Text -> Guidance)
                text_input = tokenizer.decode(context_ids[0], skip_special_tokens=True)
                ar_input_ids = aligner.src.encode(text_input, return_tensors="pt").to(device)
                
                # Get logits in Guidance space
                ar_out_logits = ar_guidance_model(ar_input_ids).logits[:, -1, :]

                # Map to Backbone space
                ar_logits = aligner.align(ar_out_logits, backbone_vocab_size)
            else:
                # 1. Get raw logits (Original behavior)
...
def main():
    ...
    # [FIX] Load tokenizer for guidance and attach aligner
    # This ensures that even if tokenizers mismatch, we can map logits correctly.
    try:
        ar_tokenizer = AutoTokenizer.from_pretrained(args.ar_guidance_model, trust_remote_code=True)
        # LogitAligner maps Guidance(Src) -> Backbone(Tgt)
        # Note: tokenizer is already defined above for the backbone model
        aligner = LogitAligner(ar_tokenizer, tokenizer)
        ar_guidance_model.logit_aligner = aligner
        print("Guidance Tokenizer loaded and LogitAligner attached.")
    except Exception as e:
